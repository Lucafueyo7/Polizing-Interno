...
Modelo rediseñado a partir del esquema actual (`prisma/schema.prisma`), aplicando: aplanado de tablas puente innecesarias (1:1 / 1:N), eliminación de campos calculados (3NF), uso de enums para valores cerrados, auditoría mínima y normalización de relaciones de pagos y siniestros como 1:N reales.
...
### 1.6 — Auditoría mínima
`created_at` y `updated_at` en `clientes`, `polizas`, `siniestros`, `pagos`, `empresas_aseguradoras`.

### 1.7 — Moneda
`polizas` y `pagos` reciben `moneda CHAR(3) DEFAULT 'ARS'`.

### 1.8 — Metadata real para documentos
`siniestro_documentos` recibe `url`, `mime_type`, `hash`. `tamano` pasa a `tamano_bytes` (Int).
...
- `pagos`: `fecha_pago >= created_at` (cuando `fecha_pago` no es null)
...
| `cliente_id` | Int | FK → `clientes.id` SetNull, NOT NULL | |
| `aseguradora_id` | Int | FK → `empresas_aseguradoras.id` SetNull, NOT NULL | |
| `tipo_seguro_id` | Int | FK → `tipos_seguro.id` SetNull, NOT NULL | |
...
| `tarjeta_circulacion_poliza` | String | nullable | solo aplica para Automotor |
...
| `descripcion_hechos` | Text | nullable | |
...
|---|---|---|---|
| `id` | Int | PK, autoincrement | |
| `cliente_id` | Int | FK → `clientes.id` SetNull, NOT NULL | quién inicia el pago |
...
**Eliminados**: `fecha_emision`, `periodo`, `monto_total` (calculado), `concepto` (vive implícito en cada póliza vinculada).
...
- `siniestros`: `fuente`
- `siniestro_documentos`: `procesado_ia`
- `pagos`: `fecha_emision`, `periodo`, `monto_total`, `concepto`
...
- `created_at` / `updated_at` en tablas raíz
- `polizas.moneda`, `pagos.moneda`
- `siniestro_documentos.url`, `mime_type`, `hash`
